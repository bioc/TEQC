...
\section{Automated html Reports}
...
The function \Rfunction{TEQCreport} provides a quality report for only one sample at a time.
Usually research projects include several samples, and it might be of interest to compare enrichment
quality among all of them. Since it is cumbersome to check the TEQC reports one by one, we provide the
function \Rfunction{multiTEQCreport}, which collects results from previously created single-sample
reports and summarizes them in a new html report. While the creation of single-sample reports is time
and memory intensive, \Rfunction{multiTEQCreport} finishes quickly, since no heavy calculations have to be
done anymore. The main input to this function are the paths to the respective \Rfunction{TEQCreport}
output folders and the sample names that shall be used in tables and plots. The results can be browsed
via the index.html file in a new \Rfunarg{destDir} output directory.\\
As an example, let's assume that we have already present TEQC reports for two samples in folders
\emph{report} and \emph{report2}, respectively. Then the multiple sample report can be created by \\

\noindent
\Rcode{> multiTEQCreport(singleReportDirs = c("report", "report2"), \\
...
The output of \Rfunction{get.targets} is of class \Rclass{GRanges} from the \Rpackage{GenomicRanges} package.
...
can be specified by the option \Rfunarg{genome}. At the moment only \Rfunarg{hg18}, \Rfunarg{hg19} and \Rfunarg{hg38} are available. The corresponding genome sizes are taken from
...
so we keep the pair IDs in the resulting \Rclass{GRanges} object. The genomic positions in our bedfile containing
...
of the \Rpackage{ShortRead} function \Rfunction{scanBam}, with flag options \Rfunarg{isUnmappedQuery = FALSE} and Rfunarg{isSecondaryAlignment = FALSE}. Note that multiple matches per read on the genome are not supported. Only primary alignments are read from the BAM file.
All other previously described options of \Rfunction{get.reads} are ignored, since genomic positions and read identifiers are taken automatically from the BAM file. 
 \\
...
a \Rclass{GRanges} object whose ranges start at the first base of the first read
...
not align to the reference). In this case a list of two \Rclass{GRanges} tables will be returned,
...
\noindent
\Rcode{> reads <- reads[!(names(reads) \%in\% names(readpairs\$singleReads)), , drop = TRUE]}\\
...
order to retrieve a reduced \Robject{reads} \Rclass{GRanges} table containing only
...
\Rclass{GRanges} table, now including as 'values' columns the average coverage
...
The resulting \Rclass{GRanges} table can be converted to a data frame, and as such easily be
...
\Rfunction{coverage.targetlength.plot}. As input a \Rclass{GRanges} targets table has
...
\begin{figure}[htb!]
\begin{center}
<<coveragetargetlength, fig=TRUE, echo=F, eps=F, width=12, height=5>>=
par(mfrow=c(1,2))
...
With the \Robject{baits} \Rclass{GRanges} table and the \Robject{coverageAll}
...
Unfortunately, it is not possible to distinguish the artefactual duplicates from the naturally occurring ones.
...
example data like single-end sequences, we are left with only \Sexpr{length(reads.collapsed)} out
of originally \Sexpr{length(reads)} reads.
...
in the example we can keep \Sexpr{length(reads.collapsed.pairs)} reads. Therefore, the
...
\noindent Lawrence M, Huber W, Pages H, Aboyoun P, Carlson M, Gentleman R,
Morgan MT, Carey VJ. Software for Computing and Annotating Genomic Ranges.
